...
export interface ParsedAssetPath {
  tenantId: string;
  module: 'projects' | 'deliverables';
  projectId: string;
  assetId: string;
  version: string;
  filename: string;
  /** Directory holding the canonical original (no trailing slash). */
  versionDir: string;
}
 ...
 */
export function parseProjectAssetPath(path: string): ParsedAssetPath | null {
  if (!path) return null;
  ...
  if (moduleName !== 'projects' && moduleName !== 'deliverables') return null;
  const filename = parts[6];
  if (!filename) return null;
  // Skip already-derived proxies under the version dir (defensive).
  if (parts[5] === 'proxies') return null;
  return {
    tenantId: parts[1],
    module: moduleName,
    projectId: parts[3],
...
export function thumbnailPath(versionDir: string, width: number): string {
  return `${versionDir}/proxies/thumb_${width}.jpg`;
}

export function pdfPreviewPath(versionDir: string): string {
