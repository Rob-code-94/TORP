...
async function loadKnownAssetPathsForTenant(tenantId: string): Promise<Set<string>> {
  ...
  try {
    ...
    assetVersions.forEach((doc) => {
      const data = doc.data() as { originalPath?: string };
      if (data?.originalPath) known.add(data.originalPath);
    });
  } catch (err) {
...
async function listTenantFiles(tenantId: string, max: number): Promise<ListedFile[]> {
...
async function reconcileTenant(tenantId: string): Promise<{ scanned: number; orphans: number }> {
