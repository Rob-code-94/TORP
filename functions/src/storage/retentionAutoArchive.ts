...
type RetentionClass = 'default' | 'finance' | 'legal_hold';
...
async function loadPolicy(tenantId: string): Promise<StoragePolicyRecord | null> {
  try {
...
async function loadRetentionOverrides(
  tenantId: string,
): Promise<Map<string, { retentionClass: RetentionClass; legalHold: boolean }>> {
  const overrides = new Map<string, { retentionClass: RetentionClass; legalHold: boolean }>();
  try {
    const db = getFirestore();
    const snap = await db
      .collection('tenants')
    ...
    snap.forEach((doc) => {
      ...
      if (!path) return;
      overrides.set(path, {
        retentionClass: (data.retentionClass as RetentionClass | undefined) || 'default',
...
async function listTenantFiles(tenantId: string, max: number): Promise<StorageFile[]> {
  ...
  for (const subdir of ['projects/', 'deliverables/']) {
    ...
    do {
      const response = (await bucket.getFiles({
        ...
        pageToken,
        maxResults: 500,
      })) as unknown as [StorageFile[], { pageToken?: string } | undefined];
      const [files, query] = response;
      for (const file of files) {
        if (out.length >= max) break;
        if (isProxyPath(file.name)) continue;
        out.push(file);
      }
...
async function writeAuditRow(tenantId: string, payload: Record<string, unknown>): Promise<void> {
  try {
    const db = getFirestore();
    await db
      .collection('tenants')
...
async function reconcileTenant(tenantId: string): Promise<ReconcileResult> {
  ...
  for (const file of files) {
    const updated = file.metadata?.updated as string | undefined;
    ...
    const retentionClass: RetentionClass =
      override?.retentionClass ?? (policy.legalHoldDefault ? 'legal_hold' : 'default');
