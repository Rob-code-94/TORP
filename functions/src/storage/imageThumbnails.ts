 ...
 */
const DEFAULT_WIDTHS = [256, 640, 1280] as const;
 ...
 */
async function renderJpegThumbnail(input: Buffer, width: number): Promise<Buffer> {
...
export const onImageAssetFinalized = onObjectFinalized(
  ...
  async (event) => {
    ...
    const tasks = DEFAULT_WIDTHS.map(async (width): Promise<ProxyResult | null> => {
    ...
    const results = (await Promise.all(tasks)).filter((r): r is ProxyResult => r !== null);
    if (results.length === 0) {
