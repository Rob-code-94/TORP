...
import type { CalendarSyncMappingDoc, SyncEventPayload } from './types.js';
 ...
 */
export async function pushEventForUid(
  env: GoogleEnv,
  uid: string,
  torpEntityId: string,
  ...
  const next: CalendarSyncMappingDoc = {
    torpEntityId,
    torpEntityType,
    uid,
    provider: 'google',
...
export async function deleteEventForUid(
  env: GoogleEnv,
  uid: string,
  torpEntityType: 'shoot' | 'meeting' | 'planner',
  torpEntityId: string,
...
function hashPayload(payload: SyncEventPayload): string {
