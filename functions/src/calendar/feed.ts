...
export interface IcsCalendarOptions {
  /** Visible name of the calendar in the user's app (X-WR-CALNAME). */
  calName: string;
  /** Description shown by some clients. */
...
/**
 * Loads all calendar events for a uid. The actual queries against
 * `shoots`, `meetings`, and `plannerItems` are executed by the caller and
 * passed in here as already-collected payloads. This keeps the ICS builder
 * pure / testable without coupling it to Firestore schema.
 */
export interface FeedEventLoader {
  loadEventsForUid(uid: string): Promise<SyncEventPayload[]>;
}
