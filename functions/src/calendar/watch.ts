...
import { getValidAccessToken, type GoogleEnv } from './google.js';
import { getFirestore } from 'firebase-admin/firestore';
import type { CalendarSyncMappingDoc } from './types.js';
...
function watchEndpoint(): string {
  const projectId = process.env.GCLOUD_PROJECT ?? '';
  const region = process.env.FUNCTION_REGION ?? 'us-central1';
  return `https://${region}-${projectId}.cloudfunctions.net/googleCalendarWebhook`;
}

async function startWatchForUid(env: GoogleEnv, uid: string): Promise<void> {
  ...
  const res = await fetch(
    `${GCAL_API_BASE}/calendars/${encodeURIComponent(conn.calendarId)}/events/watch`,
    {
      method: 'POST',
      headers: {
...
async function reconcileGoogleDeletesForUid(env: GoogleEnv, uid: string): Promise<void> {
  const conn = await getConnection(uid);
  if (!conn || conn.status !== 'connected') return;
  const token = await getValidAccessToken(env, uid);
  const params = new URLSearchParams({ showDeleted: 'true', singleEvents: 'true' });
  if (conn.nextSyncToken) params.set('syncToken', conn.nextSyncToken);
  else {
  ...
  const data = (await res.json()) as {
    items?: { id?: string; status?: string; extendedProperties?: { private?: { torpEntityKey?: string } } }[];
...
export const startCalendarWatchForMe = onCall<Record<string, never>>(
...
export const googleCalendarWebhook = onRequest(
  ...
  async (req, res) => {
    try {
      ...
      logger.error('calendar.webhook.failed', e as Error);
 ...
 */
export const refreshCalendarWatchChannels = onSchedule(
  { schedule: 'every 24 hours', secrets: CALENDAR_SECRETS },
  async () => {
    ...
    for (const doc of snap.docs) {
      const data = doc.data() as { uid?: string; watchExpiresAtMs?: number | null };
      if (!data.uid) continue;
      if (data.watchExpiresAtMs && data.watchExpiresAtMs > cutoff) continue;
      try {
        await startWatchForUid(env, data.uid);
      } catch (e) {
        logger.warn('calendar.watch.refresh.failed', { uid: data.uid, error: (e as Error).message });
      }
    }
  },
);
