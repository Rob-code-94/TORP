...
function isPdfContentType(contentType, filename) {
    const lower = (contentType || '').toLowerCase();
    if (lower === 'application/pdf' || lower === 'application/x-pdf')
        return true;
...
async function renderFirstPage(pdfBytes) {
    ...
    try {
        ...
        logger.error('pdf-preview: @napi-rs/canvas not available', { err: err?.message });
        return null;
    }
...
export const onPdfAssetFinalized = onObjectFinalized({
    ...
    if (!parsed)
        return;
    if (!isPdfContentType(contentType, parsed.filename))
        return;
