...
import { getConnection, patchConnection, setConnection, } from './firestore.js';
/**
...
export function buildOAuthAuthorizeUrl(env, state) {
...
async function postFormToGoogle(url, params) {
...
export async function exchangeCodeForTokens(env, code) {
    const data = await postFormToGoogle(GOOGLE_TOKEN_URL, {
        code,
        client_id: env.clientId,
...
export async function revokeToken(token) {
    try {
        ...
        logger.warn('google.revoke.failed', e);
    }
}
...
export async function persistConnection(env, uid, tokens, email) {
...
export function decodeOAuthState(encoded) {
    const json = Buffer.from(encoded, 'base64url').toString('utf8');
    return JSON.parse(json);
}
function payloadToResource(payload) {
...
export async function upsertEventForUser(env, uid, calendarId, payload, externalEventId) {
    let token;
    try {
        token = await getValidAccessToken(env, uid);
    }
    catch (e) {
        return { ok: false, error: e.message, transient: false };
    }
    ...
    if (!created.id)
        return { ok: false, error: 'google_no_event_id', transient: true };
    return { ok: true, externalEventId: created.id, calendarId };
}
export async function deleteEventForUser(env, uid, calendarId, externalEventId) {
    let token;
    try {
        token = await getValidAccessToken(env, uid);
    }
    ...
    const res = await fetch(`${GCAL_API_BASE}/calendars/${encodeURIComponent(calendarId)}/events/${encodeURIComponent(externalEventId)}`, {
        method: 'DELETE',
...
export async function readFreeBusy(env, uid, calendarId, windowStartIso, windowEndIso) {
