...
import { deleteConnection, findFeedTokenDocByHash, getConnection, getFeedTokenDoc, listConnections, listSyncMappingsForUid, patchConnection, setFeedTokenDoc, COL, } from './firestore.js';
import { buildIcsFeed } from './feed.js';
import { loadCrewEvents } from './loader.js';
import { buildOAuthAuthorizeUrl, decodeOAuthState, encodeOAuthState, exchangeCodeForTokens, fetchUserInfo, persistConnection, readFreeBusy, revokeToken, } from './google.js';
...
function googleEnvFromRuntime(redirectUri) {
    return {
...
export const getMyCalendarConnection = onCall({ secrets: CALENDAR_SECRETS }, async (req) => {
    const uid = requireUid(req);
    const doc = await getConnection(uid);
    return { connection: publicConnection(doc) };
});
export const setMyCalendarPreferences = onCall({ secrets: CALENDAR_SECRETS }, async (req) => {
    const uid = requireUid(req);
    const existing = await getConnection(uid);
    if (!existing)
        throw new HttpsError('failed-precondition', 'Connect Google Calendar first.');
    const patch = {};
    if (typeof req.data?.pushEnabled === 'boolean')
        patch.pushEnabled = req.data.pushEnabled;
    if (typeof req.data?.freebusyEnabled === 'boolean')
        patch.freebusyEnabled = req.data.freebusyEnabled;
    if (Object.keys(patch).length === 0)
        return { connection: publicConnection(existing) };
    await patchConnection(uid, patch);
    const updated = await getConnection(uid);
    return { connection: publicConnection(updated) };
});
export const googleCalendarOAuthStart = onCall({ secrets: CALENDAR_SECRETS }, async (req) => {
    const uid = requireUid(req);
...
export const googleCalendarOAuthCallback = onRequest({ secrets: CALENDAR_SECRETS }, async (req, res) => {
    try {
        ...
        if (!stored.exists || stored.data().nonce !== state.nonce) {
        ...
        logger.error('calendar.oauth.callback.failed', e);
        res.status(500).send('Could not finish sign-in. Please try again.');
    }
});
export const disconnectGoogleCalendar = onCall({ secrets: CALENDAR_SECRETS }, async (req) => {
    const uid = requireUid(req);
    ...
    if (conn.refreshTokenEnc) {
        try {
            ...
            logger.warn('calendar.disconnect.revoke.failed', e);
        }
    }
...
function buildFeedUrlForToken(plaintext) {
    const projectId = process.env.GCLOUD_PROJECT ?? '';
    const region = process.env.FUNCTION_REGION ?? 'us-central1';
    return `https://${region}-${projectId}.cloudfunctions.net/calendarFeed/${encodeURIComponent(plaintext)}.ics`;
}
async function ensureFeedToken(uid) {
    const existing = await getFeedTokenDoc(uid);
    if (existing)
        return { doc: existing, plaintext: null };
...
export const getMyFeedToken = onCall({ secrets: CALENDAR_SECRETS }, async (req) => {
    const uid = requireUid(req);
    const { doc, plaintext } = await ensureFeedToken(uid);
    if (plaintext)
        return { url: buildFeedUrlForToken(plaintext), rotatedAt: doc.rotatedAt };
...
export const rotateMyFeedToken = onCall({ secrets: CALENDAR_SECRETS }, async (req) => {
    const uid = requireUid(req);
    const generated = newFeedToken(uid);
    const doc = {
        uid,
 ...
 */
export const calendarFeed = onRequest({ secrets: CALENDAR_SECRETS, cors: false }, async (req, res) => {
    try {
        ...
        logger.error('calendar.feed.failed', e);
        res.status(500).send('Could not load feed.');
    }
});
export const listOrgCalendarConnections = onCall({ secrets: CALENDAR_SECRETS }, async (req) => {
    requireAdmin(req);
...
async function resyncAllForUid(uid) {
    const conn = await getConnection(uid);
    ...
    await Promise.allSettled(events.map(async (e) => {
        const [type, id] = e.torpEntityKey.split(':');
        try {
...
export const forceResyncForUser = onCall({ secrets: CALENDAR_SECRETS }, async (req) => {
    requireAdmin(req);
 ...
 */
export const retryMyCalendarSync = onCall({ secrets: CALENDAR_SECRETS }, async (req) => {
    const uid = requireUid(req);
    const conn = await getConnection(uid);
    if (!conn)
        throw new HttpsError('failed-precondition', 'Connect Google Calendar first.');
 ...
 */
export const getMyCalendarFreeBusy = onCall({ secrets: CALENDAR_SECRETS }, async (req) => {
    const uid = requireUid(req);
    const conn = await getConnection(uid);
    ...
    if (!startIso || !endIso)
        throw new HttpsError('invalid-argument', 'startIso and endIso required');
    const projectId = process.env.GCLOUD_PROJECT ?? '';
    const region = process.env.FUNCTION_REGION ?? 'us-central1';
    const env = {
        clientId: GOOGLE_OAUTH_CLIENT_ID.value(),
    ...
    try {
        ...
        logger.warn('calendar.freebusy.failed', { uid, error: e.message });
        return { busy: [], available: false };
    }
});
